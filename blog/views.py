...
def index(request):
    ...
    most_popular_posts = Post.objects.popular().prefetch_related('author').prefetch_related(
        Prefetch('tags', queryset=tags_with_annotation))[:5].fetch_with_comments_count()

    fresh_posts = Post.objects.order_by('published_at').prefetch_related('author').prefetch_related(
        Prefetch('tags', queryset=tags_with_annotation)).fetch_with_comments_count()
